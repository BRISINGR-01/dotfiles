...
rofiConfig="$HOME/dotfiles/rofi/applets/webSearch.rasi"
...
main() {
  ...
  platform=$( (gen_list) | rofi -dmenu -matching fuzzy -no-custom -location 0 -p "Platform: " -config ${rofiConfig})

  if [[ -n "$platform" ]]; then
    query=$( (echo) | rofi -dmenu -matching fuzzy -location 0 -p "Search: " -config ${rofiConfig})
